...
volatile sig_atomic_t is_sigint = FALSE;
...
int main(int argc, char const *argv[]) {
    ...
    if (pthread_barrier_init(&barrier, NULL, worker_num + 1) != 0) {
        display_error("Error initializing the barrier");
        return 1;
    }
...
void handle_fifo(char *src, char *dest) {
    ...
    while ((curr_occupancy == buffer_size || available_workers == 0) && is_finished == FALSE) {
...
void *worker(void *arg) {
    while (TRUE) {
        pthread_mutex_lock(&general_mutex);
        while (is_finished == FALSE && buffer[0].src_fd == -1) {
...
void sigint_handler(int signo) {
    ...
    is_sigint = TRUE;
    pthread_cond_broadcast(&buffer_not_empty_cond);
    pthread_cond_broadcast(&buffer_not_full_cond);
    pthread_mutex_unlock(&general_mutex);
